function [Sx,Sy,Sz,wavefield,simtime]=FDwave_calculation_3Delastic_2N_vTI(varargin) %src_i,dN_W,dN_SS,plotON)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load([wfp,[filesep,'model']],'dx','dy','dz','nx','ny','nz','C11','C12','C13','C22','C23','C33');

load([wfp,[filesep,'derived_param']],'mu','muxyz','lam','lamu','bx','by','bz','C44','C55','C66')
...
[vx_x, vy_x, vz_x, txx_x, tyy_x, tzz_x, txy_x, txz_x       ] = deal(zeros(nz,nx,ny));
[vx_y, vy_y, vz_y, txx_y, tyy_y, tzz_y, txy_y,        tyz_y] = deal(zeros(nz,nx,ny));
[vx_z, vy_z, vz_z, txx_z, tyy_z, tzz_z,        txz_z, tyz_z] = deal(zeros(nz,nx,ny));


nframes=length(1:dN_W:N);      
wavefield=zeros(nz,nx,nframes);
...
switch M %different M denotes accuracy (up to tenth-order) of the spatial derivative operators
    case 4
...
for t=1:N -3              
    ...
    txy_x(srcnz,srcnx,srcny)=txy_x(srcnz,srcnx,srcny)+(-MT(1,2)/2)*src(t);
    txy_y(srcnz,srcnx,srcny)=txy_y(srcnz,srcnx,srcny)+(-MT(1,2)/2)*src(t);
    
    txz_x(srcnz,srcnx,srcny)=txz_x(srcnz,srcnx,srcny)+(-MT(1,3)/2)*src(t);
    txz_z(srcnz,srcnx,srcny)=txz_z(srcnz,srcnx,srcny)+(-MT(1,3)/2)*src(t);
    
    tyz_y(srcnz,srcnx,srcny)=tyz_y(srcnz,srcnx,srcny)+(-MT(2,3)/2)*src(t);
    tyz_z(srcnz,srcnx,srcny)=tyz_z(srcnz,srcnx,srcny)+(-MT(2,3)/2)*src(t);
    ...
    dxvx = Dxbm(vx,ii, jj, kk)/dx;
    
    dyvy = Dybm(vy,ii, jj, kk)/dy;
    
    dzvz = Dzbm(vz,ii, jj, kk)/dz;
    %txx(3:nz-2, 3:nx-2, 3:ny-2) = txx(3:nz-2, 3:nx-2, 3:ny-2)  +  dt*( lamu(3:nz-2, 3:nx-2, 3:ny-2).*dxvx + lam(3:nz-2, 3:nx-2, 3:ny-2).*(dyvy + dzvz) );
    txx_x(ii, jj, kk) =  (Pmlxn.*txx_x(ii, jj, kk)  +  dt* C11(ii, jj, kk).*dxvx)./Pmlxd;
    txx_y(ii, jj, kk) =  (Pmlyn.*txx_y(ii, jj, kk)  +  dt* C12(ii, jj, kk).*dyvy)./Pmlyd;
    txx_z(ii, jj, kk) =  (Pmlzn.*txx_z(ii, jj, kk)  +  dt* C13(ii, jj, kk).*dzvz)./Pmlzd;
    
    %tyy(3:nz-2, 3:nx-2, 3:ny-2) = tyy(3:nz-2, 3:nx-2, 3:ny-2)  +  dt*( lam(3:nz-2, 3:nx-2, 3:ny-2).*(dxvx + dzvz) + lamu(3:nz-2, 3:nx-2, 3:ny-2).*dyvy );
    tyy_x(ii, jj, kk) = (Pmlxn.*tyy_x(ii, jj, kk)  +  dt* C12(ii, jj, kk).*dxvx)./Pmlxd;
    tyy_y(ii, jj, kk) = (Pmlyn.*tyy_y(ii, jj, kk)  +  dt* C22(ii, jj, kk).*dyvy)./Pmlyd;
    tyy_z(ii, jj, kk) = (Pmlzn.*tyy_z(ii, jj, kk)  +  dt* C23(ii, jj, kk).*dzvz)./Pmlzd;
    
    %tzz(3:nz-2, 3:nx-2, 3:ny-2) = tzz(3:nz-2, 3:nx-2, 3:ny-2)  +  dt*( lam(3:nz-2, 3:nx-2, 3:ny-2).*(dxvx + dyvy)+ lamu(3:nz-2, 3:nx-2, 3:ny-2).*dzvz );
    tzz_x(ii, jj, kk) = (Pmlxn.*tzz_x(ii, jj, kk)  +  dt* C13(ii, jj, kk).*dxvx)./Pmlxd;
    tzz_y(ii, jj, kk) = (Pmlyn.*tzz_y(ii, jj, kk)  +  dt* C23(ii, jj, kk).*dyvy)./Pmlyd;
    tzz_z(ii, jj, kk) = (Pmlzn.*tzz_z(ii, jj, kk)  +  dt* C33(ii, jj, kk).*dzvz)./Pmlzd;
    
    %txy(3:nz-2, 3:nx-2, 3:ny-2) = txy(3:nz-2, 3:nx-2, 3:ny-2)  +  dt*muxyz(3:nz-2, 3:nx-2, 3:ny-2).*(  Dyfm(vx,nx,ny,nz)/dy + Dxfm(vy,nx,ny,nz)/dx   );
    txy_x(ii, jj, kk) = (Pmlxn.*txy_x(ii, jj, kk)  +  dt*C66(ii, jj, kk).*Dxfm(vy,ii, jj, kk)/dx)./Pmlxd;
    txy_y(ii, jj, kk) = (Pmlyn.*txy_y(ii, jj, kk)  +  dt*C66(ii, jj, kk).*Dyfm(vx,ii, jj, kk)/dy)./Pmlyd;
    
    %txz(3:nz-2, 3:nx-2, 3:ny-2) = txz(3:nz-2, 3:nx-2, 3:ny-2)  +  dt*muxyz(3:nz-2, 3:nx-2, 3:ny-2).*(  Dzfm(vx,nx,ny,nz)/dz + Dxfm(vz,nx,ny,nz)/dx   );
    txz_x(ii, jj, kk) = (Pmlxn.*txz_x(ii, jj, kk)  +  dt*C55(ii, jj, kk).*Dxfm(vz,ii, jj, kk)/dx)./Pmlxd;
    txz_z(ii, jj, kk) = (Pmlzn.*txz_z(ii, jj, kk)  +  dt*C55(ii, jj, kk).*Dzfm(vx,ii, jj, kk)/dz)./Pmlzd;
    
    %tyz(3:nz-2, 3:nx-2, 3:ny-2) = tyz(3:nz-2, 3:nx-2, 3:ny-2)  +  dt*muxyz(3:nz-2, 3:nx-2, 3:ny-2).*(  Dzfm(vy,nx,ny,nz)/dz + Dyfm(vz,nx,ny,nz)/dy   );
    tyz_y(ii, jj, kk) = (Pmlyn.*tyz_y(ii, jj, kk)  +  dt*C44(ii, jj, kk).*Dyfm(vz,ii, jj, kk)/dy)./Pmlyd;
    tyz_z(ii, jj, kk) = (Pmlzn.*tyz_z(ii, jj, kk)  +  dt*C44(ii, jj, kk).*Dzfm(vy,ii, jj, kk)/dz)./Pmlzd;
   ...
   %% ABL boundary
    ...
    txx(ii, jj, kk) = txx(ii, jj, kk)  +  dt*( C11(ii, jj, kk).*dxvx + C12(ii, jj, kk).*dyvy+C13(ii, jj, kk).*dzvz );
    tyy(ii, jj, kk) = tyy(ii, jj, kk)  +  dt*( C12(ii, jj, kk).*dxvx + C22(ii, jj, kk).*dyvy+C23(ii, jj, kk).*dzvz );
    tzz(ii, jj, kk) = tzz(ii, jj, kk)  +  dt*( C13(ii, jj, kk).*dxvx + C23(ii, jj, kk).*dyvy+C33(ii, jj, kk).*dzvz );
    txy(ii, jj, kk) = txy(ii, jj, kk)  +  dt*C66(ii, jj, kk).*(  Dyfm(vx,ii, jj, kk)/dy + Dxfm(vy,ii, jj, kk)/dx   );
    txz(ii, jj, kk) = txz(ii, jj, kk)  +  dt*C55(ii, jj, kk).*(  Dzfm(vx,ii, jj, kk)/dz + Dxfm(vz,ii, jj, kk)/dx   );
    tyz(ii, jj, kk) = tyz(ii, jj, kk)  +  dt*C44(ii, jj, kk).*(  Dzfm(vy,ii, jj, kk)/dz + Dyfm(vz,ii, jj, kk)/dy   );
...
save(str,'Sx','Sy','Sz','dN_SS','dt','N','M','dx','dy','dz','nx','ny','nz','-v7.3');
