function FDwave_model_n_3Dlayers_FulAni(varargin)  % wave_type,   dx,dy,dz,HV_ratio,tl,  vpv,vsv,rhov,qpv,qsv)

% This function can create a 3D model consisting of 'n' horizontal layers in full anisotropic case.
% Complete Syntax:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    elseif strcmp(wave_type,'elastic')
        ...
        disp([str0,'Density of layer(s)  : ',num2str(rhov),str9])
        disp([str0,'Elastic tensor of layer(s)  : ',num2str(reshape(C{1},1,size(C{1},1)*size(C{1},2))),str10])
...
elseif strcmp(wave_type,'elastic')
    for i=1:length(vpv)
        ...
        vsm(zn(i):zn(i+1),:,:) = vsv(i);
        rhom(zn(i):zn(i+1),:,:)= rhov(i);
        C11(zn(i):zn(i+1),:,:)= C{i}(1,1);
        C12(zn(i):zn(i+1),:,:)= C{i}(1,2);
        C13(zn(i):zn(i+1),:,:)= C{i}(1,3);
        C14(zn(i):zn(i+1),:,:)= C{i}(1,4);
        C15(zn(i):zn(i+1),:,:)= C{i}(1,5);
        C16(zn(i):zn(i+1),:,:)= C{i}(1,6);
        C22(zn(i):zn(i+1),:,:)= C{i}(2,2);
        C23(zn(i):zn(i+1),:,:)= C{i}(2,3);
        C24(zn(i):zn(i+1),:,:)= C{i}(2,4);
        C25(zn(i):zn(i+1),:,:)= C{i}(2,5);
        C26(zn(i):zn(i+1),:,:)= C{i}(2,6);
        C33(zn(i):zn(i+1),:,:)= C{i}(3,3);
        C34(zn(i):zn(i+1),:,:)= C{i}(3,4);
        C35(zn(i):zn(i+1),:,:)= C{i}(3,5);
        C36(zn(i):zn(i+1),:,:)= C{i}(3,6);
        C44(zn(i):zn(i+1),:,:)= C{i}(4,4);
        C45(zn(i):zn(i+1),:,:)= C{i}(4,5);
        C46(zn(i):zn(i+1),:,:)= C{i}(4,6);
        C55(zn(i):zn(i+1),:,:)= C{i}(5,5);
        C56(zn(i):zn(i+1),:,:)= C{i}(5,6);
        C66(zn(i):zn(i+1),:,:)= C{i}(6,6);
    end
...
elseif strcmp(wave_type,'viscoelastic')
    ...
    save(str,'dx','dy','dz','nx','ny','nz','vpm','vsm','rhom','C11','C12','C13','C14','C15','C16','C22','C23','C24',...
        'C25','C26','C33','C34','C35','C36','C44','C45','C46','C55','C56','C66','qpm','qsm','wave_type','m_name');
else
